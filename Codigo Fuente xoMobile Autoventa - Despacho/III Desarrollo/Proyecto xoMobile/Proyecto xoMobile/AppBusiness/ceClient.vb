...
#Region " URL DE CONEXION "
    Public Function configuraParametros() As Boolean
        Try

            '--- Abrir pantalla de configuracion de parametros
            Cursor.Current = Cursors.WaitCursor
            Dim ofrmSincronizar As New frmImportar_
            Dim oBitacora As New BitacoraBL
            ofrmSincronizar.TabControl1.TabPages.RemoveAt(0)
            ofrmSincronizar.TabControl1.TabPages.RemoveAt(0)
            ofrmSincronizar.TabControl1.TabPages.RemoveAt(0)
            ofrmSincronizar.TabControl1.SelectedIndex = 0
            ofrmSincronizar.ShowDialog()
            Cursor.Current = Cursors.Default
            If Not ofrmSincronizar.conexion_exitosa Then Return False
            Return True
        Catch ex As Exception
            Return False
        End Try
    ...
    Public Function construyeURL(ByVal estoyFuera As Boolean, ByRef service As xoservice.xotest, ByVal servidor As String, ByVal ruta As String)

        '--- Verificar ip del servidor
        If (servidor Is Nothing Or servidor.Length = 0) Then
            MsgBox("No fue posible reconocer la direccion IP del servidor")
            Return False
        End If

        '--- Verificar ip del servidor
        If (ruta Is Nothing Or ruta.Length = 0) Then
            MsgBox("El código de ruta no es valido.")
            Return False
        End If


        '--- Cheque estoy fuera de la licorera, indica el protocolo de seguridad
        If estoyFuera Then
            service.Url = "https://" & servidor & "/xoservice/xotest.asmx"
            'service.Url = "https://" & id_glo_server + "/xoservice/xoTest.asmx"
            id_glo_estoy_fuera = True
        Else
            service.Url = "http://" & servidor & "/xoservice/xotest.asmx"
